...
import {TemplatesProceduresHelper} from "../helpers/template-procedures-helper";

@Component({
  selector: 'templates-procedures-section',
...
export class ProceduresSectionComponent implements OnInit {
  ...
  removeSection() {
    ...
    const procedureLength = this.procedureList.controls.length;
    for (let i = procedureLength - 1; i >= 0; i--) {
      this.procedureList.removeAt(i);
    }
  }
