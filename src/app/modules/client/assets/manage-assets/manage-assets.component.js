...
var ManageAssetsComponent = (function () {
    function ManageAssetsComponent(formBuilder, apiService, toastr, router, route) {
        ...
        this.imageUploadUrl = environment_1.environment.baseUrl;
        this.documentUploadUrl = environment_1.environment.baseUrl;
        this.image = {
            objectURL: '',
            notDefault: false,
            deleted: false
        ...
        this.assetForm = this.formBuilder.group({
            ...
            serial: [''],
            validity_time: ['', [forms_1.Validators.required]],
            integration_date: [new Date(), [forms_1.Validators.required]],
            end_service_life_date: [{ value: new Date(), disabled: true }],
            warranty_date: [],
            disincorporation_date: [],
            brand_id: ['', [forms_1.Validators.required]],
            category_id: ['', [forms_1.Validators.required]],
            workplace_id: ['', [forms_1.Validators.required]],
            status_id: ['', [forms_1.Validators.required]],
            model_id: [''],
            worker: ['', forms_1.Validators.required],
            sub_category_id: [''],
    ...
    ManageAssetsComponent.prototype.addCustomFields = function (custom_fields_config) {
        ...
        if (custom_fields_config) {
            var customFields = custom_fields_config.map(function (customField) {
                var formObject = {};
                formObject.value = ['', customField.required ? forms_1.Validators.required : null];
    ...
    ManageAssetsComponent.prototype.ngOnInit = function () {
        ...
        this.apiService.all('client/assets/config').subscribe(function (config) {
            ...
            if (config.data.sku_type === 'mask') {
                _this.skuMask = maskParser_1.parseMask(config.data.sku_mask);
            }
        });
    ...
    ManageAssetsComponent.prototype.onSubmit = function (form, $event) {
        ...
        if (!validationError) {
            ...
            delete data.worker;
            var formData = objectToFormData_1.objectToFormData(data);
...
exports.ManageAssetsComponent = ManageAssetsComponent;
